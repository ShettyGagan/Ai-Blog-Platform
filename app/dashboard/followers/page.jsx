...
const UserCard = ({
  ...
  return (
    <div className="flex items-center justify-between p-4 bg-gradient-to-r from-slate-900/50 to-blue-900/20 rounded-xl border border-cyan-500/20 hover:border-cyan-400/40 transition-all hover:shadow-lg hover:shadow-cyan-500/10 group">
      {/* Avatar + Info */}
      <div className="flex items-center space-x-3">
        <Link href={`/${user.username}`}>
          <div className="relative w-12 h-12 cursor-pointer group-hover:scale-110 transition-transform">
            {user.imageUrl ? (
              <Image
                src={user.imageUrl}
                alt={user.name}
              ...
              <div className="w-full h-full rounded-full bg-gradient-to-br from-cyan-600 to-blue-600 flex items-center justify-center text-base font-bold shadow-lg">
        ...
        <Link href={user.username ? `/${user.username}` : ""}>
          <div className="cursor-pointer">
            <p className="font-semibold text-white hover:text-cyan-300 transition-colors">
              {user.name}
            </p>
            {user.username && (
      ...
      {variant === "follower" ? (
        !user.followsBack && (
          <Button
            ...
            variant="outline"
            size="sm"
            className="border-cyan-500 text-cyan-400 hover:bg-cyan-500 hover:text-white transition-all"
          >
            {isLoading ? (
